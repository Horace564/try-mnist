...
def main():
    ...
    train_data = [(x.to(device), y.to(device)) for x, y in get_data(is_train=True)]
    test_data = [(x.to(device), y.to(device)) for x, y in get_data(is_train=False)]
    ...
    print("Total training time(GPU) is ",round(time.time()-start_time, 2), "s")
